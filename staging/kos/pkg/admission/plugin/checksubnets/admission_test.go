...
func TestChecksubnetsAdmissionPlugin(t *testing.T) {
	var scenarios = []struct {
		informersOutput        network.FischerList
		admissionInput         network.Flunder
		...
		// a flunder with a name that appears on a list of disallowed flunders must be banned
		{
			...
			admissionInputKind:     network.Kind("Flunder").WithVersion("version"),
			admissionInputResource: network.Resource("flunders").WithVersion("version"),
		...
		// but the kind passed in is not a flunder thus the whole request is accepted
		{
			...
			admissionInputKind:     network.Kind("NotFlunder").WithVersion("version"),
			admissionInputResource: network.Resource("notflunders").WithVersion("version"),
			admissionMustFail:      false,
		},
	}
	...
	for index, scenario := range scenarios {
		func() {
			...
			target, err := checksubnets.New()
			if err != nil {
				t.Fatalf("scenario %d: failed to create checksubnets admission plugin due to = %v", index, err)
			}

			targetInitializer := networkinitializer.New(informersFactory)
			targetInitializer.Initialize(target)

			err = admission.ValidateInitialization(target)
			if err != nil {
				t.Fatalf("scenario %d: failed to initialize checksubnets admission plugin due to =%v", index, err)
			}
			...
			if !scenario.admissionMustFail && err != nil {
				t.Errorf("scenario %d: checksubnets admission plugin returned unexpected error = %v", index, err)
			}
		}()
	}
}
